...
char i2c_slaveAddress = 0x48;
int TXByteCtr;
unsigned char PRxData;
unsigned int Count;
int Rx = 2;     // we need to read 7 bytes from RTC then we will reset it.
...
int main(void)
{
    ...
    P1DIR |= BIT0;
    P1SEL  |= BIT6 + BIT7;                     // Assign I2C pins to USCI_B0        P3.1 and P3.2 I2C Data & Clock
    ...
    while(1){
        ...
        if(pinState == 0)
        {
            //Transmit process
            //Rx = 0;
            if(Rx == 3)         // we need to read 7 bytes from RTC then we will reset it.
            {
                TXByteCtr = 1;
                I2C_transmit_master();
                Rx=1;
